...
export default class SliceService {
    ...
    handlePythonMicroService(): Promise<(object)> {
        ...
        return new Promise(async (resolve, reject) => {
            const pythonSliceMicroService = spawn(config.PYTHON_INTERPRETER_PATH, [config.PYTHON_SLICE_SERVICE_PATH, `./uploads/${this.fileToSlice}`, this.scalingFormat, this.slicingFormat]);
